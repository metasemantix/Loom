...
describe("read-only machine access",()=>{
  ...
  it("versions Agent compression independently and keeps revision alignment truthful",async()=>{
    ...
    const conflict=await SELF.fetch(`${origin}/api/me/documents/${documentId}/metadata`,{method:"PUT",headers:{cookie:alice.cookie,origin,"content-type":"application/json"},body:JSON.stringify({title:stale.document.title,logicalPath:stale.document.logical_path,visibility:stale.document.visibility,compression:"wrong binding",sourceVersionId:source})});expect(conflict.status).toBe(409);
