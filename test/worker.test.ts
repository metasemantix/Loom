...
describe("scheduled account lifecycle", () => {
  ...
  it("rejects late cancellation and finalizes irreversibly and idempotently", async () => {
    const alice=await participant("deleteme"),bob=await participant("bobstays"),carol=await participant("ordinarymember"),documentId=await create(alice.cookie,"public");
