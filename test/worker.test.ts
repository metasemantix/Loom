...
describe("browser UI", () => {
  ...
  it("preserves compact project administration and an intact picker during link confirmation", async () => {
    ...
    expect(html).toContain("else if(j.canUnarchive)");expect(html).toContain("cannot be unarchived while its owner has account deletion pending");expect(html).toContain("Recover ownership and unarchive");
...
describe("scheduled account lifecycle", () => {
  ...
  it("rejects late cancellation and finalizes irreversibly and idempotently", async () => {
    const alice=await participant("delete-me"),bob=await participant("bobstays"),carol=await participant("ordinarymember"),documentId=await create(alice.cookie,"public");
    ...
    const recovery=await SELF.fetch(`${origin}/api/projects/${made.project.id}/unarchive`,{method:"POST",headers:{cookie:bob.cookie,origin}});expect(recovery.status).toBe(200);expect(await recovery.json<any>()).toMatchObject({project:{status:"active",ownerParticipantId:bob.id,recoveredOwnership:true}});
