...
describe("project-native documents",()=>{
  ...
  it("schedules owner-confirmed deletion, enforces the deadline, and finalizes a content-free shell",async()=>{const alice=await participant("alice"),bob=await participant("bob"),id=await project(alice);await invite(id,alice,bob);await SELF.fetch(`${origin}/api/projects/${id}/members/${bob.id}`,{method:"PUT",headers:{cookie:alice.cookie,origin,"content-type":"application/json"},body:JSON.stringify({role:"admin"})});const made=await native(id,alice),doc=(await made.json<any>()).document.id;const mutate=(actor:{cookie:string},title:string)=>SELF.fetch(`${origin}/api/projects/${id}/deletion`,{method:"POST",headers:{cookie:actor.cookie,origin,"content-type":"application/json"},body:JSON.stringify({title})});expect((await mutate(bob,"Native")).status).toBe(403);expect((await mutate(alice,"native")).status).toBe(400);const scheduled=await mutate(alice,"Native");expect(scheduled.status).toBe(201);const body=await scheduled.json<any>();expect(new Date(body.project.deletionDueAt).getTime()-new Date(body.project.deletionScheduledAt).getTime()).toBe(3*86400_000);expect((await SELF.fetch(`${origin}/api/projects/${id}/unarchive`,{method:"POST",headers:{cookie:bob.cookie,origin}})).status).toBe(409);expect((await SELF.fetch(`${origin}/api/projects/${id}/deletion`,{method:"DELETE",headers:{cookie:bob.cookie,origin}})).status).toBe(403);await env.DB.prepare(`UPDATE projects SET deletion_due_at=? WHERE id=?`).bind("2026-01-01T00:00:00.000Z",id).run();expect((await SELF.fetch(`${origin}/api/projects/${id}/deletion`,{method:"DELETE",headers:{cookie:alice.cookie,origin}})).status).toBe(409);const {finalizeDueProjects}=await import("../src/project-deletion");await finalizeDueProjects(env,new Date("2026-01-01T00:00:00.000Z"));expect(await env.DB.prepare(`SELECT lifecycle_state,deletion_finalized_at FROM projects WHERE id=?`).bind(id).first<any>()).toMatchObject({lifecycle_state:"shell",deletion_finalized_at:"2026-01-01T00:00:00.000Z"});expect((await env.DB.prepare(`SELECT count(*) count FROM document_versions WHERE document_id=?`).bind(doc).first<any>()).count).toBe(0);expect((await env.DB.prepare(`SELECT count(*) count FROM project_document_revision_shell WHERE document_id=?`).bind(doc).first<any>()).count).toBe(1);expect((await env.DB.prepare(`SELECT count(*) count FROM project_members WHERE project_id=?`).bind(id).first<any>()).count).toBe(0);const html=await SELF.fetch(`${origin}/projects`,{headers:{cookie:bob.cookie}}).then(r=>r.text());const script=html.match(/<script>([\s\S]*?)<\/script>/)?.[1]??"";expect(()=>new Function(script)).not.toThrow()});
