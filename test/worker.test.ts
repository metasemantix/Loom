...
import { COMPRESSION_GENERATION_TARGET_CHARACTERS, COMPRESSION_MAX_CHARACTERS, COMPRESSION_PROMPT, COMPRESSION_PROMPT_VERSION, compressionRequest, validateCompression } from "../src/compression";
...
const substantialIdeaCollection=(source:string,minimumLength:number)=>{
  const items:Array<Record<string,unknown>>=[];
  while(projection(source,"idea_collection",{items}).length<minimumLength)items.push({name:`Independent idea ${items.length+1}`,kind:items.length%2?"agent communication":"hostile-input handling",gist:`Source item ${items.length+1} describes a distinct retrieval technique, its operating constraints, and the circumstances in which a reader should consult the full source.`,topics:items.length%2?["agents","communication","coordination"]:["security","untrusted input","isolation"]});
  return projection(source,"idea_collection",{items});
};
...
describe("migration upgrade safety",()=>{
  it("preserves populated 0015 keyboard history while adding continuity columns",()=>{const result=(globalThis as any).__loomMigrationRegression, before=result.keyboardBefore,after=result.keyboardAfter;expect(after.chain).toEqual(before.chain);expect(after.message).toEqual({...before.message,message_index:1});expect(after.capability).toEqual({...before.capability,predecessor_capability_id:null});expect(after.event).toEqual(before.event);expect(result.foreignKeyErrors).toEqual([])});
  it("preserves existing corpus state while migrating machine credentials and compression",()=>{const result=(globalThis as typeof globalThis & {__loomMigrationRegression:{before:unknown;after:unknown;afterProjectDeletionMigration:unknown;migratedCredential:unknown;migratedProseCompression:unknown;migratedStructuredColumns:unknown;compressionRowsBeforeExpansion:unknown;compressionRowsAfterExpansion:unknown;selectedCompressionAfterExpansion:unknown;foreignKeyErrors:unknown[]}}).__loomMigrationRegression;expect(result.after).toEqual(result.before);expect(result.afterProjectDeletionMigration).toEqual(result.before);expect(result.migratedCredential).toEqual({id:"mac_migration",checkin_enabled:0});expect(result.migratedProseCompression).toMatchObject({id:"cmp_legacy_doc_migration",text:"Historical prose compression",source_version_id:null,actor_type:null,actor_id:null,created_at:null,prompt_version:null});expect(result.migratedStructuredColumns).toEqual({text:"Historical prose compression",prompt_version:null,artifact_format:"legacy-prose",schema_version:null,artifact_json:null});expect(result.compressionRowsAfterExpansion).toEqual(result.compressionRowsBeforeExpansion);expect(result.selectedCompressionAfterExpansion).toEqual({selected_compression_revision_id:"cmp_structured_migration"});expect(result.foreignKeyErrors).toEqual([])})
});
...
describe("read-only machine access",()=>{
  ...
  it("saves and exposes validated structured v3 compressions with truthful lifecycle state",async()=>{
    ...
    for(const compression of ["not json","x".repeat(COMPRESSION_MAX_CHARACTERS+1),JSON.stringify({schema_version:1}),projection("wrong")]){const denied=await SELF.fetch(`${origin}/api/me/documents/${documentId}/metadata`,{method:"PUT",headers:{cookie:alice.cookie,origin,"content-type":"application/json"},body:JSON.stringify({title:first.document.title,logicalPath:first.document.logical_path,visibility:first.document.visibility,compression,sourceVersionId:source})});expect(denied.status).toBe(400);expect((await denied.json<any>()).error.message).toMatch(/compression|source_revision/)}
  ...
  it("treats 4,000 characters as guidance and 8,000 as the shared hard save limit",async()=>{
    expect(COMPRESSION_GENERATION_TARGET_CHARACTERS).toBe(4000);expect(COMPRESSION_MAX_CHARACTERS).toBe(8000);
    const alice=await participant("compression_limits"),documentId=await create(alice.cookie),owned=(await SELF.fetch(`${origin}/api/documents/${documentId}`,{headers:{cookie:alice.cookie}}).then(r=>r.json<any>())).document;
    const normal=substantialIdeaCollection(owned.current_version_id,2500);expect(normal.length).toBeGreaterThan(2000);expect(normal.length).toBeLessThanOrEqual(COMPRESSION_GENERATION_TARGET_CHARACTERS);
    const participantSave=await SELF.fetch(`${origin}/api/me/documents/${documentId}/metadata`,{method:"PUT",headers:{cookie:alice.cookie,origin,"content-type":"application/json"},body:JSON.stringify({title:owned.title,logicalPath:owned.logical_path,visibility:owned.visibility,compression:normal,sourceVersionId:owned.current_version_id})});expect(participantSave.status).toBe(200);
    const projectResponse=await SELF.fetch(`${origin}/api/projects`,{method:"POST",headers:{cookie:alice.cookie,origin,"content-type":"application/json"},body:JSON.stringify({name:"Compression limits",readAudience:"members_and_agents"})}),projectId=(await projectResponse.json<any>()).project.id;
    const nativeResponse=await SELF.fetch(`${origin}/api/projects/${projectId}/native-documents`,{method:"POST",headers:{cookie:alice.cookie,origin,"content-type":"application/json"},body:JSON.stringify({title:"Dense ideas",content:"Many independently meaningful source ideas.",contentType:"text/plain"})}),nativeId=(await nativeResponse.json<any>()).document.id;
    const native=(await SELF.fetch(`${origin}/api/project-documents/${nativeId}`,{headers:{cookie:alice.cookie}}).then(r=>r.json<any>())).document;
    const headroom=substantialIdeaCollection(native.current_version_id,6000);expect(headroom.length).toBeGreaterThan(COMPRESSION_GENERATION_TARGET_CHARACTERS);expect(headroom.length).toBeLessThanOrEqual(COMPRESSION_MAX_CHARACTERS);
    const projectSave=await SELF.fetch(`${origin}/api/project-documents/${native.id}/metadata`,{method:"PUT",headers:{cookie:alice.cookie,origin,"content-type":"application/json"},body:JSON.stringify({title:native.title,logicalPath:native.logical_path,compression:headroom,sourceVersionId:native.current_version_id})});expect(projectSave.status).toBe(200);
    for(const [url,document,compression] of [[`/api/me/documents/${documentId}/metadata`,owned,substantialIdeaCollection(owned.current_version_id,COMPRESSION_MAX_CHARACTERS+1)],[`/api/project-documents/${native.id}/metadata`,native,substantialIdeaCollection(native.current_version_id,COMPRESSION_MAX_CHARACTERS+1)]] as const){expect(compression.length).toBeGreaterThan(COMPRESSION_MAX_CHARACTERS);const response=await SELF.fetch(origin+url,{method:"PUT",headers:{cookie:alice.cookie,origin,"content-type":"application/json"},body:JSON.stringify({title:document.title,logicalPath:document.logical_path,visibility:document.visibility,compression,sourceVersionId:document.current_version_id})});expect(response.status).toBe(400);expect((await response.json<any>()).error.message).toContain("8,000");}
  });
  ...
  expect(COMPRESSION_PROMPT_VERSION).toBe("compression-prompt-v3");const request=compressionRequest("Current title","Current full body",owned.document.current_version_id);expect(request).toContain(`AUTHORITATIVE SOURCE REVISION ID:\n${owned.document.current_version_id}`);expect(request).toContain("aim for 4,000 characters or fewer");expect(request).toContain("do not add detail merely because additional space is available");expect(request).toContain("absolute maximum for generation is 5,000 characters");expect(request).toContain("Never meet the 4,000-character target by truncating JSON");expect(request).toContain("collapsing independently meaningful idea_collection items");
  for(const path of [`/documents/${documentId}`,`/project-documents/${nativeId}`]){const html=await SELF.fetch(origin+path,{headers:{cookie:alice.cookie}}).then(r=>r.text()),script=html.match(/<script>([\s\S]*?)<\/script>/)?.[1]??"";expect(()=>new Function(script)).not.toThrow();expect(html).toContain("Structured v3");expect(html).toContain("Aim for 4,000 characters or fewer; 8,000 characters is the absolute maximum");expect(script).toContain("maxLength=8000");expect(script).toContain("+' / 8,000'");expect(script).toContain("replaceAll('[source revision ID]',d.current_version_id)");expect(script).toContain("addEventListener('input',count)");expect(script).not.toContain("fetch("+"compressionPrompt");}
});
