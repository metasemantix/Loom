...
describe("read-only machine access",()=>{
  it("creates a one-time secret, reads the live scoped corpus, audits, and revokes immediately",async()=>{
    ...
    expect((await SELF.fetch(`${origin}/api/agent/documents`,{method:"POST",headers})).status).toBe(405);expect((await SELF.fetch(`${origin}/api/agent/me`,{headers:{authorization:"Bearer malformed"}})).status).toBe(401);
