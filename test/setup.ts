...
for(const sql of [migration,productSlice,humanProjects,projectLifecycle,accountLifecycle,minimalTombstones,projectNativeDocuments])for(const statement of statements(sql))await env.DB.prepare(statement).run();
