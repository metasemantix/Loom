...
for(const sql of [migration,productSlice,humanProjects,projectLifecycle,accountLifecycle])for(const statement of statements(sql))await env.DB.prepare(statement).run();
