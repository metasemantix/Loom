...
for(const sql of [migration,productSlice,humanProjects,projectLifecycle,accountLifecycle,minimalTombstones])for(const statement of statements(sql))await env.DB.prepare(statement).run();
