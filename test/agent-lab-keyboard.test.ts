...
async function menu(url="/agent-lab/keyboard/enter"):Promise<Menu>{
  const response=await get(url),body=await response.text(),lines=body.split("\n"),links=new Map<string,string>();
  for(const line of lines){const match=line.match(/^([a-z]+): (https:\/\/\S+)$/);if(match)links.set(match[1],match[2])}
  return {response,body,value:lines[1]??"",links};
}
...
describe("verbatim-link compositional keyboard",()=>{
  it("enters with a fresh empty persisted message and one deterministic 28-link menu",async()=>{
    const first=await menu(),second=await menu(),labels=[..."abcdefghijklmnopqrstuvwxyz","space","done"];
    ...
    expect(await messageFor(cap(first))).toMatchObject({value:"",symbol_count:0,completed_at:null});
  ...
  it("rejects symbol 129 without consumption while allowing done from the same menu",async()=>{
    ...
    const done=await get(current.links.get("done")!);expect(done.status).toBe(200);expect((await done.text()).match(/^read: https:\/\/\S+\n$/)).toBeTruthy();
  });

  it("completes nonempty and empty messages, then reads the exact value only once",async()=>{
    for(const expected of ["pebble",""]){
      let current=await menu();for(const letter of expected)current=await choose(current,letter);
      const stale=current.links.get("a")!,before=await messageFor(cap(current)),done=await get(current.links.get("done")!),doneBody=await done.text(),readUrl=doneBody.match(/^read: (\S+)$/m)?.[1];
      expect(done.status).toBe(200);expect([...doneBody.matchAll(/https:\/\/\S+/g)]).toHaveLength(1);expect(readUrl).toBeTruthy();
      ...
      const read=await get(readUrl!);expect(read.status).toBe(200);expect(await read.text()).toBe(`value:\n${expected}\n`);expect((await get(readUrl!)).status).toBe(403);
    }
  });
  ...
  it("rejects malformed, unknown, expired, wrong-operation, and cross-message tokens without successors",async()=>{
    ...
    const completed=await get(first.links.get("done")!),readUrl=(await completed.text()).match(/^read: (\S+)$/m)![1],readToken=new URL(readUrl).searchParams.get("cap")!,before=(await env.DB.prepare(`SELECT count(*) count FROM agent_lab_keyboard_capabilities`).first<{count:number}>())!.count;
