...
describe("verbatim-link compositional keyboard",()=>{
  ...
  it("completes nonempty and empty messages, then reads the exact value only once",async()=>{
    for(const expected of ["pebble",""]){
      ...
      const read=await get(readUrl);expect(read.status).toBe(200);expect(read.headers.get("content-type")).toBe("text/plain; charset=utf-8");expect(read.headers.get("cache-control")).toBe("no-store");expect(await read.text()).toBe(`value:\n${expected}\n`);expect((await get(readUrl)).status).toBe(403);
    }
  });
