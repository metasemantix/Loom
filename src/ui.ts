...
export function spacePage(displayName: string, participantId: string): Response {
  return new Response(`<!doctype html><title>My Space — Loom</title>${style}<main><h1>${escapeHtml(displayName)}’s space</h1><p>Your stable participant ID: <code>${participantId}</code></p><p><a href="/api/me/export" download>Export my space</a> as a portable ZIP archive.</p><form id="new"><label>Title<input name="title" maxlength="120" required></label><label>Kind<select name="kind"><option value="document">Document</option><option value="profile">Profile</option><option value="introduction">Introduction</option></select></label><label>Visibility<select name="visibility"><option value="private">Private</option><option value="public">Public</option></select></label><label>Markdown<textarea name="content" required></textarea></label><button>Create document</button></form><section id="documents" aria-live="polite"></section></main><script>
