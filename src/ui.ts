...
async function showHistory(d,panel){
  ...
  for(const e of j.events||[]){const h=document.createElement('h4'),meta=document.createElement('p'),body=document.createElement('pre');h.textContent='Document details changed';meta.textContent='By '+revisionAuthor(e)+' · '+new Date(e.created_at).toLocaleString();body.textContent=JSON.stringify(e.changes,null,2);section.append(h,meta,body)}for(const v of j.versions){const h=document.createElement('h4'),meta=document.createElement('p'),date=document.createElement('time'),body=document.createElement('pre');h.textContent='Revision '+v.version_number;meta.className='meta';meta.append('By '+revisionAuthor(v)+' · ');date.dateTime=v.created_at;date.textContent=new Date(v.created_at).toLocaleString();meta.append(date);body.textContent=v.content;section.append(h,meta,body)}
...
export function controlRoomPage(): Response {
  return new Response(`<!doctype html><title>Control Room — Loom</title>${style}<main><nav><a href="/me">My Space / Documents</a> · <a href="/projects">Projects</a></nav><h1>Control Room / Profile</h1><dl id="identity"></dl><form id="profile"><label>Loom display name<input name="displayName" maxlength="80" required></label><button>Save display name</button></form></main><script>const identity=document.querySelector('#identity'),form=document.querySelector('#profile');async function load(){const r=await fetch('/api/me/profile'),j=await r.json();identity.innerHTML='<dt>Stable participant ID</dt><dd><code>'+j.participant.id+'</code></dd><dt>Provenance lookup ID</dt><dd><code>'+j.participant.lookupId+'</code></dd><dt>Connected providers</dt><dd>'+(j.identities.map(i=>i.provider+' ('+i.provider_user_id+')').join(', ')||'None')+'</dd>';form.displayName.value=j.participant.displayName}form.onsubmit=async e=>{e.preventDefault();const r=await fetch('/api/me/profile',{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify({displayName:form.displayName.value})});if(r.ok)load();else alert((await r.json()).error.message)};load()</script>`, { headers: { "content-type": "text/html; charset=utf-8", "content-security-policy": "default-src 'self'; style-src 'unsafe-inline'; script-src 'unsafe-inline'; frame-ancestors 'none'" } });
}

export function projectsPage(): Response {
  return new Response(`<!doctype html><title>Projects — Loom</title>${style}<main><nav><a href="/me">My Space / Documents</a> · <a href="/control-room">Control Room / Profile</a></nav><h1>Projects</h1><p>Projects reference participant-owned documents; they do not copy or transfer ownership. Adding a document grants the project’s read audience access, but never write or delete authority.</p><form id="new"><label>Name<input name="name" required maxlength="120"></label><label>Read audience<select name="readAudience"><option value="members_and_agents">Members and agents</option><option value="agents_only">Agents only (documents hidden from human browsing)</option></select></label><button>Create project</button></form><section id="projects"></section></main><script>const section=document.querySelector('#projects');async function load(){const j=await(await fetch('/api/projects')).json();section.replaceChildren(...j.projects.map(p=>{const a=document.createElement('article');a.innerHTML='<h2>'+p.name+'</h2><p>Read audience: '+p.read_audience+' · Role: '+p.role+'</p><button type="button">Open project</button>';a.querySelector('button').onclick=()=>openProject(p.id,a);return a}))}async function openProject(id,a){const j=await(await fetch('/api/projects/'+id)).json();a.querySelectorAll('section').forEach(x=>x.remove());const s=document.createElement('section');s.innerHTML='<h3>Members</h3><pre>'+JSON.stringify(j.members,null,2)+'</pre><h3>Linked documents</h3><pre>'+(j.documentsHiddenFromHumans?'Hidden from human members by agents_only policy':JSON.stringify(j.documents,null,2))+'</pre><form><label>Add existing participant ID<input name="participantId"></label><button>Add member</button></form><form><label>Link one of your document IDs<input name="documentId"></label><button>Link my document</button></form>';const forms=s.querySelectorAll('form');forms[0].onsubmit=async e=>{e.preventDefault();await fetch('/api/projects/'+id+'/members',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({participantId:e.target.participantId.value})});openProject(id,a)};forms[1].onsubmit=async e=>{e.preventDefault();if(!confirm('This grants the project-defined read audience access. Ownership and write control stay with you.'))return;await fetch('/api/projects/'+id+'/documents',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({documentId:e.target.documentId.value})});openProject(id,a)};a.append(s)}document.querySelector('#new').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target),r=await fetch('/api/projects',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({name:f.get('name'),readAudience:f.get('readAudience')})});if(r.ok){e.target.reset();load()}else alert((await r.json()).error.message)};load()</script>`, { headers: { "content-type": "text/html; charset=utf-8", "content-security-policy": "default-src 'self'; style-src 'unsafe-inline'; script-src 'unsafe-inline'; frame-ancestors 'none'" } });
}
