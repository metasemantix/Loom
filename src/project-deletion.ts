...
export async function finalizeDueProjects(env:Env,clock=new Date()):Promise<number>{
  ...
  for(const project of due.results){
    ...
    const results=await env.DB.batch([
      ...
      // A terminal shell cannot retain a live grant. Keep the credential row as
      // provenance, but make every still-active bearer unusable atomically with
      // finalization rather than relying only on a later lifecycle read.
      env.DB.prepare(`UPDATE project_machine_credentials SET revoked_at=COALESCE(revoked_at,?),revoked_by_participant_id=NULL WHERE project_id=? AND EXISTS(SELECT 1 FROM projects WHERE id=? AND lifecycle_transition_id=?)`).bind(now,project.id,project.id,transition),
