...
- Treat package-manager updates as execution-environment maintenance, not product changes. After an npm tooling update, verify that repository files such as `package.json` and the lockfile were not modified solely by that update.
- If npm crashes internally (including errors such as `Cannot read properties of null (reading 'edgesOut')`), check/update npm to the current compatible stable release and retry the failed command once before concluding that Loom or the sandbox is at fault.
- If checking or updating npm requires network access that the environment denies, do not repeatedly retry it. Continue with the available toolchain where possible and report the limitation explicitly.
